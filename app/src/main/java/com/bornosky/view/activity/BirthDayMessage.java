...
public class BirthDayMessage extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.birth_day_message);
        setTitle("BirthDayMessage");
        ...
        spinnerSession.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (!s1.contentEquals("Select session")) {
                    ...
                    ArrayAdapter<String> messageTypeAdapter = new ArrayAdapter<>(BirthDayMessage.this, R.layout.spinner_item, messageTypeArray);
        ...
        checkAll.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked) {
                    ...
                    DataItemAdapter adapter = new DataItemAdapter(BirthDayMessage.this, dataItemList);
                    RecyclerView recyclerView = (RecyclerView) findViewById(R.id.rvItems);
                    recyclerView.setAdapter(adapter);
                } else {
           ...
           /*     checkedPhoneList = DataItemAdapter.phones;
                if (checkedPhoneList.size() > 0) {
                    for (int i = 0; i < checkedPhoneList.size(); i++) {
                        Toast.makeText(BirthDayMessage.this, "your number " + checkedPhoneList.get(i), Toast.LENGTH_SHORT).show();
...
*//*
                ...
                if (checkedAllPhoneList.size() > 0) {
                    for (int i = 0; i < checkedAllPhoneList.size(); i++) {
                        Toast.makeText(BirthDayMessage.this, "your number " + checkedAllPhoneList.get(i), Toast.LENGTH_SHORT).show();
                        SmsManager sms = SmsManager.getDefault();
                        message = etmessage.getText().toString();
